...
get-geo-assets:
	@echo ""
	@echo "Creating directory $(GEO_ASSETS_DIR)..."
ifeq ($(OS),Windows_NT)
	if not exist "$(GEO_ASSETS_DIR)" $(MKDIR) "$(GEO_ASSETS_DIR)" || (echo "Error creating directory" & exit 1)
else
	@$(MKDIR) "$(GEO_ASSETS_DIR)" || (echo "Error creating directory"; exit 1)
endif
	@echo ""
	@echo "Downloading geoip.db..."
	curl -L https://github.com/SagerNet/sing-geoip/releases/latest/download/geoip.db -o "$(GEO_ASSETS_DIR)$(SEP)geoip.db" || (echo "Error downloading geoip.db"; exit 1)
	@echo "Downloading geosite.db..."
	curl -L https://github.com/SagerNet/sing-geosite/releases/latest/download/geosite.db -o "$(GEO_ASSETS_DIR)$(SEP)geosite.db" || (echo "Error downloading geosite.db"; exit 1)
